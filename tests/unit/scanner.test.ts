...
describe('FileScanner', () => {
  ...
  beforeAll(async () => {
    ...
    await fs.writeFile(
      path.join(testDir, 'src/utils.py'),
      'def helper():\n    return "helper"\n'
    );

    await fs.writeFile(
      path.join(testDir, 'src/app.js'),
      'console.log("JavaScript file - should be ignored");'
    );

    await fs.writeFile(
      path.join(testDir, '__pycache__/main.cpython-39.pyc'),
      'compiled python file'
    );

    await fs.writeFile(
      path.join(testDir, 'venv/lib/something.py'),
      'virtual env file'
    );
